...
２. 　本アプリは、ユーザーの操作により本サービスに利用された顔写真（以下、「顔写真」といいます。）、および顔写真から検出される顔の特徴点（ランドマーク）や顔・眉周辺の位置情報（以下、これらを総称して「顔データ」といいます。）を取り扱います。顔データの解析処理はユーザーの端末上で行われ、専用サーバーその他の外部サーバーへ送信または保存されることはありません。

3.　顔の特徴点（ランドマーク）等の解析のため一時的に生成される顔データは、処理に必要な間のみ端末上で使用され、処理終了後は保持されません。ユーザーの操作により本サービスの過去の画像一覧画面に表示される顔写真は端末内にのみ保存され、ユーザーが本アプリ内の履歴として保持している間、または本アプリを端末から削除するまで保持されます。
...
２. 顔写真および顔データを用いて、顔・眉周辺の位置を検出し、眉デザインの診断、配置、試着および調整等の本サービスの機能を提供するため
...
　２.顔写真および顔データを、Firebase、AdMobその他の第三者に提供または共有することはありません。
...
4.本アプリ内に保存された顔写真等のデータは、本アプリを端末から削除することにより削除できます。ユーザーが端末の写真ライブラリに保存した画像については、端末の写真アプリから削除できます。
